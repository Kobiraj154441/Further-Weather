...
protocol FurtherWeatherDataDelegate {
...
class FurtherWeatherManager {
    ...
    var weatherDataDelegate: FurtherWeatherDataDelegate?
    ...
    func fetchURL(with latitute: CLLocationDegrees, longitude: CLLocationDegrees) {
    ...
    func parseJSON(weatherData: Data) -> FurtherWeatherModel? {
        let decoder = JSONDecoder()
        do {
            let decodedData = try decoder.decode(FurtherWeatherData.self, from: weatherData)
            let cityName = decodedData.name
            let conditionId = decodedData.weather[0].id
            let temperature = decodedData.main.temp
            
            return FurtherWeatherModel(cityName: cityName, temp: temperature, conditionId: conditionId)
        } catch {
            self.weatherDataDelegate?.didFailWithError(error: error)
            return nil
        }
